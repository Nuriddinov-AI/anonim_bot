...
async def start(message: Message, state: FSMContext):
    ...
    if len(args) > 1:
        ...
        if target and target['telegram_id'] != message.from_user.id:
            ...
            await message.answer(
                "😅 O'zingizning havolangizga kira olmaysiz!",
...
async def receive_question(message: Message, state: FSMContext):
    ...
    if message.from_user.username:
        from_info = "@" + message.from_user.username
    else:
        from_info = message.from_user.full_name or "Noma'lum"

    q_id = await send_question(target_id, from_info, message.text)
    await state.clear()
...
async def my_link(message: Message):
    ...
    await message.answer(
        "🔗 <b>Shaxsiy havolangiz:</b>\n\n"
        "<code>" + link + "</code>\n\n"
        "Bu havolani Instagram, Telegram yoki boshqa joylarda ulashing!",
        parse_mode="HTML"
    )
...
async def stats(message: Message):
    ...
    await message.answer(
        "📊 <b>Statistikangiz</b>\n\n"
        "📨 Jami savollar: <b>" + str(total) + "</b>\n"
        "✅ Javob berilgan: <b>" + str(answered) + "</b>\n"
        "⏳ Javobsiz: <b>" + str(unanswered) + "</b>",
        parse_mode="HTML"
    )
...
async def show_users(message: Message):
    ...
    for i, u in enumerate(users, 1):
        ...
        text += (
            ...
            "   👤 Username: " + username + "\n"
            "   📱 Tel: " + phone + "\n"
            "   🆔 ID: <code>" + str(u['telegram_id']) + "</code>\n"
            "   📅 " + str(u['created_at'])[:16] + "\n"
            "━━━━━━━━━━━━━━━━━\n"
        )
